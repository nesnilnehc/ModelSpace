...
### 可传播认知资产（2026-03-20 更新）

| 方向 | 描述 | 估算 | 价值 | 状态 |
|------|------|------|------|------|
| **G 落地页 URL** | 每个模型固定 URL，视频 CTA 指向，承载完整定义/坐标/关系 | small | 高 | done |
| **D 抖音竖卡** | 9:16 单模型卡：名称、定义、坐标、CTA「详情见评论」 | small | 高 | done |
| **E 创作者脚本模板** | 按 v2 schema 导出：钩子(3s)→定义(10s)→例子(15s)→关联(5s) | small | 高 | done |
| **F 系列化路径** | 如 MECE→Issue Tree→5 Whys 做 3 集系列（P2） | medium | 中 | optional |
...
| G 落地页 URL、D 抖音竖卡、E 脚本模板 | 见上表 | done |
...
| 优先级 | 项 | 理由 |
|--------|-----|------|
| 1 | 体系化学习（路径+手册导出） | D/E/G 已完成；衔接落地页，形成学习闭环 |
| 2 | domain/model-data 抽离 | 架构演进已完成 |
| 3 | 详情 IA 改版 | 独立、用户可感知 |
| 4 | 局部截图 / 海报模板 | 按业务需求择机 |
