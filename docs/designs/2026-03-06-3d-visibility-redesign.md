...
**Date:** 2026-03-06
**Status:** Proposed
**Goal:** 解决对象过多带来的视觉混乱，优化浏览体验，提供“总览-聚焦”的认知路径。
...
- **交互模式**: **B (Overview First)**
  - **默认状态 (Overview Mode)**:
    - 所有 Cell 显示 **徽章 (Badge)**: 包含该 Cell 对象数量、类别颜色。
    - **隐藏** 所有单个对象的 Label (或降低不透明度至 0.2)。
    - **点击空白处** 保持此状态。
  - **聚焦状态 (Focus Mode)**:
    - **点击 Cell 徽章** 或 **选中 Cell 内任一对象** 进入。
    - **高亮** 该 Cell 内的所有对象 (Opacity 1.0)。
    - **淡化** 其他 Cell (Opacity 0.1, Label 隐藏)。
    - **显示** 该 Cell 内对象的 Label 与连线。
...
- **布局**: **手风琴 (Accordion)**
  ...
  - **记忆功能**: 记录用户手动展开/折叠的状态 (SessionStorage)，下次打开同类对象时保持偏好。
...
- [ ] 点击 Cell 徽章，平滑过渡到该 Cell 高亮状态，相关对象文字清晰。
- [ ] 点击空白处，平滑退回总览模式。
...
- **依赖**: 无额外库。
- **风险**: Cell 徽章遮挡模型点击。
- **缓解**: 徽章 `pointer-events: auto`，点击徽章视为选中 Cell；对象 `raycast` 优先级高于徽章背景。
